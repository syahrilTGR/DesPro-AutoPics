...
*   **Logic**: Validasi akun, status, dan saldo secara realtime via Firebase, kontrol gate non-blocking.
...
*   **Tugas**: Membaca streaming gambar dari ESP32-CAM, melakukan segmentasi ROI (Region of Interest) pada koordinat slot (mobil/motor), menganalisis okupansi menggunakan akselerasi GPU (PyTorch), serta mengunggah status slot dan ringkasan ketersediaan ke Firebase secara realtime.

### D. User Interface (Mobile App)
*   **Fitur**:
    *   **Real-time Counter**: Membaca data jumlah slot kosong untuk mobil dan motor secara live dari Firebase.
...
## 2. Struktur Data Firebase Realtime Database (RTDB) - AKTIF & SINKRON
...
1.  **Vision Loop**: ESP32-CAM $\rightarrow$ Stream Video $\rightarrow$ Python Vision Engine $\rightarrow$ Analisis ROI GPU $\rightarrow$ Update Firebase `parkir/slots` & `parkir/ringkasan`.
